    ...
    <body>
        <div id="full">
            ...
            <div class="profileaction">
                ...
                <div class="tblstatus">
                    <div Style="margin-top: 25px;margin-left:65px;margin-bottom: 30px">
                    
                        <form name="frm" action="remove_tblbooking" method="post" onsubmit="return tblcanvalid();">
                            
                    <table class="tblstatuss" border="0" width="90%" id="tata" Style="text-align: center;">
                       <%
                           Class.forName("com.mysql.jdbc.Driver");
                           Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/thelittlejoint?useSSL=false","root","root");
                           Statement st=con.createStatement();
                       ...
                       <input type="hidden" name="ids" />
                        ...
                        <%
                            ResultSet rs1 = st.executeQuery("select * from tbl_booking where user_id='"+n+"' ;");
                            while(rs1.next()){
                        %>
                        <tr class="selectrow">
                            <td><%=rs1.getString("booking_id")%></td>
                            <td><%=rs1.getString("Date_booking")%></td>
                            <td><%=rs1.getString("no_person")%></td>
                            <td><%=rs1.getString("Time")%></td>
                            <td><input type="submit" value="Cancel booking" id="<%=rs1.getString("booking_id")%>" onclick="document.frm.ids.value=this.id;"/></td>
                        <script>
                            function tblcanvalid(){
                                if(confirm('Do you really want to cancel your table booking...')){
                                    return true;
